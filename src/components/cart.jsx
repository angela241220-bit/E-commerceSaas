...
export default function Cart({ Icon, variant = "outline", }) {
    ...
    return (<Link href="/cart">
      <Button variant={variant} size="icon" className="relative">
        ...
        {cartItemsCount > 0 && (<span className="absolute -top-2 -right-2 bg-primary text-primary-foreground text-xs rounded-full h-4 w-4 flex items-center justify-center">
            {cartItemsCount}
          </span>)}
      </Button>
    </Link>);
