...
const CartPage = ({ store }) => {
    ...
    return (<section className="flex flex-col min-h-screen bg-[#fff] md:pt-16 pt-24 dark:bg-[#252525]">
      <Navbar storeSlug={store.slug} storeName={store.name} storeId={store.id}/>
              ...
              <AlertDialog>
                ...
                <AlertDialogContent>
                  ...
                  <AlertDialogFooter>
                    ...
                    <AlertDialogAction className="bg-red-500 hover:bg-red-600" onClick={clearCart}>
                      Clear Cart
                    </AlertDialogAction>
                  </AlertDialogFooter>
                </AlertDialogContent>
                  ...
                  <div className="flex gap-4">
                    ...
                    <div className="flex-1">
                      <div className="flex justify-between">
                        ...
                        <button onClick={() => removeItem(item.id)} className="text-gray-900 hover:text-red-500 dark:text-gray-400">
                      ...
                      <div className="flex items-center mt-4">
                        <div className="flex items-center border rounded">
                          <button onClick={() => updateItemQuantity(item.id, Math.max(1, item.quantity - 1))} className="p-2 hover:bg-gray-100 dark:hover:bg-gray-800 text-gray-900 dark:text-gray-400" disabled={item.quantity <= 1}>
                            <Minus className="w-4 h-4"/>
                          </button>
                          <span className="px-4 py-2 text-center min-w-[3rem] text-gray-900 dark:text-gray-400">
                            {item.quantity}
                          </span>
                          <button onClick={() => updateItemQuantity(item.id, item.quantity + 1)} className="p-2 hover:bg-gray-100 dark:hover:bg-gray-800 text-gray-900 dark:text-gray-400" disabled={item.inStock !== undefined &&
                item.quantity >= item.inStock}>
                            <Plus className="w-4 h-4"/>
                          </button>
                        </div>
                        {item.trackQuantity && item.inStock !== undefined && (<span className="ml-4 text-sm text-gray-900 dark:text-gray-400">
