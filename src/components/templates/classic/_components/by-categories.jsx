...
export default function ByCategories({ storeId }) {
        ...
        {Array.from({ length: 5 }).map((_, i) => (<div key={i}>
            ...
            <ScrollArea>
              <div className="flex space-x-4 pb-4">
                {Array.from({ length: 10 }).map((_, j) => (<div key={j} className="w-64 flex-shrink-0">
    ...
    if (error || !categories || categories.length === 0)
        return null;
    return (<div className="space-y-8">
      {categories.map((category) => category.products.length > 0 ? (<div key={category.id}>
            ...
            <ScrollArea>
              <div className="flex space-x-4 pb-4">
                {category.products.map((product) => (<div key={product.id} className="w-44 flex-shrink-0">
