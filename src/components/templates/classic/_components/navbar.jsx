...
export default function Navbar({ storeSlug, storeName, storeId, }) {
    ...
    return (<header className="w-full bg-white/60 dark:bg-black/70 text-black dark:text-white backdrop-blur-lg py-2 px-4 fixed top-0 left-0 right-0 z-50 flex flex-col gap-1">
      <nav className="flex items-center justify-between gap-5">
        ...
        <form onSubmit={handleSearch} className="w-2/3 max-w-2xl hidden md:block">
          <Input placeholder="Search for products" className="w-full rounded-full bg-slate-200 dark:bg-slate-800 dark:text-white text-black" value={query} onChange={(e) => setQuery(e.target.value)}/>
        </form>
      ...
      <form onSubmit={handleSearch} className="w-full max-w-2xl block md:hidden">
        <Input placeholder="Search for products" className="w-full rounded-full bg-slate-200 dark:bg-slate-800 dark:text-white text-black" value={query} onChange={(e) => setQuery(e.target.value)}/>
      </form>
    </header>);
}
