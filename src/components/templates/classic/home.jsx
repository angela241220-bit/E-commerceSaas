...
export default function Home({ store }) {
    return (<section className="min-h-screen bg-[#fff] md:pt-16 pt-24 flex flex-col dark:bg-[#252525]">
      ...
      <section className="flex-1 flex flex-col gap-10 lg:justify-between lg:min-h-[calc(100vh-5rem)]">
        {store.customisations[0].bannerSettings.show && (<div className="container mx-auto pt-0 md:pt-5">
            <div className="bg-transparent/50 backdrop-blur-lg">
              <Banners banners={store.banners}/>
            </div>
          </div>)}
        {store.customisations[0].productWheelSettings.show && (<div className="container mx-auto">
            <ProductWheel storeId={store.id} circleTime={store.customisations[0].productWheelSettings.circleTime} productCount={store.customisations[0].productWheelSettings.productCount} categoryId={store.customisations[0].productWheelSettings.categoryId || "all"}/>
          </div>)}
      </section>
