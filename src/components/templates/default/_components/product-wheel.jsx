...
import { getSavedProductIds, saveProduct, unsaveProduct, } from "@/actions/store/public/saved/products";
import Image from "next/image";
function ProductWheelSkeleton() {
    return (<div className="relative w-full py-2 md:py-8 px-2 md:px-8">
      ...
      <div className="w-full max-w-7xl mx-auto animate-pulse">
        ...
        <div className="flex justify-center mt-4 sm:mt-6 gap-1 sm:gap-1.5">
          {[...Array(5)].map((_, index) => (<div key={index} className={`h-1 sm:h-1.5 rounded-full ${index === 0 ? "w-4 sm:w-6 bg-muted" : "w-1 sm:w-1.5 bg-muted/50"}`}/>))}
        </div>
      </div>
    </div>);
}
function ProductWheel({ storeId, circleTime = 3, productCount = 6, categoryId = "all", }) {
    const { data: productList, isLoading: isLoadingProducts, error, } = useQuery({
        queryKey: ["featured-products"],
        queryFn: () => getFeaturedProducts({ storeId, limit: productCount, categoryId }),
        staleTime: 1000 * 60 * 5,
    ...
    return (<div className="relative w-full py-2 md:py-8 px-2 md:px-8">
      ...
      <Carousel className="w-full max-w-7xl mx-auto" plugins={[
        ...
        <CarouselContent>
          {products?.map((product) => {
            ...
            const isWishlistActionLoading = wishlistLoading[product.id] ?? false;
            return (<CarouselItem key={product.slug} className="lg:basis-full">
                <div className="relative w-full overflow-hidden rounded-2xl p-4 sm:p-6 lg:p-10 bg-gradient-to-br from-background to-muted/50 border border-border/40 shadow-lg">
                  <div className="flex flex-row justify-between items-center gap-4 sm:gap-8 lg:gap-12">
                    {/* Product Details */}
                    <div className="flex-1 space-y-3 sm:space-y-4 lg:space-y-6">
                      <div className="space-y-1 sm:space-y-2">
                        <Link href={`/products/${product.slug}`} className="group transition-all duration-300">
                      ...
                      <div className="flex items-baseline gap-2 sm:gap-3">
                        ...
                        {product.slashedFrom && (<span className="line-through text-muted-foreground text-xs sm:text-sm lg:text-base">
                            {formatPrice(product.slashedFrom, "en-NG", "NGN")}
                          </span>)}
                      </div>

                      <div className="flex items-center gap-2">
                        {product.trackQuantity && (<span className={`inline-flex items-center px-2 sm:px-3 py-0.5 sm:py-1 rounded-full text-xs font-medium ${product.inStock > 0
                      ...
                      <div className="flex flex-col gap-2 sm:gap-4 items-start">
                        ...
                        <Link href={`/products/${product.slug}`} className="inline-flex items-center text-xs sm:text-sm font-medium text-primary hover:text-primary/80 transition-colors">
                          View Details
                    ...
                    <div className="relative flex-shrink-0">
                      <Link href={`/products/${product.slug}`} className="block group relative overflow-hidden rounded-lg sm:rounded-xl bg-background/50 p-1 sm:p-2 transition-all duration-300 hover:shadow-xl">
                        ...
                        <Button variant="secondary" size="icon" className="absolute top-3 right-3 z-10 h-8 w-8 rounded-full bg-white/80 text-black shadow-md transition-all hover:bg-white opacity-0 group-hover:opacity-100" onClick={(e) => {
                    e.preventDefault();
                    e.stopPropagation();
                    handleWishlistClick(product.id);
                }} disabled={isWishlistActionLoading}>
                          <Heart className={`h-4 w-4 transition-colors ${isSaved
      ...
      {/* Carousel Indicators */}
      <div className="flex justify-center mt-4 sm:mt-6 gap-1 sm:gap-1.5">
        {products?.map((_, index) => (<div key={index} className={`h-1 sm:h-1.5 rounded-full transition-all duration-300 ${index === 0
