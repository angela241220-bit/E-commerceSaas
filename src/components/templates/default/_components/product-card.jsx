...
const ProductCard = ({ product, isInitiallySaved = false, }) => {
    ...
    return (<div className="group overflow-hidden border-none shadow-sm hover:shadow-md transition-all duration-300 bg-card rounded-md" onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)}>
      <div className="p-0 relative">
        ...
        {isOnSale && (<Badge variant="destructive" className="absolute top-2 left-2">
            Sale
          </Badge>)}

        <Button variant="secondary" size="icon" className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity h-8 w-8 bg-white/80 hover:bg-white text-black" onClick={handleWishlist} disabled={isWishlistLoading}>
          <Heart className={`h-4 w-4 ${isSaved ? "fill-red-500 text-red-500" : "text-black"}`}/>
      ...
      <div className="p-3">
        ...
        <div className="flex items-baseline gap-2">
          ...
          {product.slashedFrom && (<span className="text-xs text-muted-foreground line-through">
      ...
      <div className="p-3 pt-0">
        {!cartItem ? (<Button size="sm" className="w-full gap-1 text-xs" onClick={handleAddToCart} disabled={product.trackQuantity && product.inStock <= 0}>
            ...
            <Button size="sm" variant="outline" className="h-8 w-8 p-0" onClick={handleDecrement}>
              <Minus className="h-3 w-3"/>
            </Button>
            <span className="text-sm font-medium">{cartItem.quantity}</span>
            <Button size="sm" variant="outline" className="h-8 w-8 p-0" onClick={handleIncrement} disabled={product.trackQuantity && cartItem.quantity >= product.inStock}>
