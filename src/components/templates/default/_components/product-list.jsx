...
function ProductList({ viewMode, searchQuery = "", storeId, categoryId, }) {
    const { data: productData, isLoading: isLoadingProducts, error, } = useQuery({
        queryKey: ["products", storeId, categoryId],
        queryFn: () => getProductsByStoreWithPagination({
            storeId,
            active: true,
            limit: 20,
            categoryId,
    ...
    if (isLoading) {
        ...
        <div className={viewMode === "grid"
                ...
                : "space-y-4"}>
          {Array.from({ length: 8 }).map((_, index) => viewMode === "grid" ? (<div key={index} className="space-y-2">
                <Skeleton className="h-48 w-full rounded-md"/>
                <Skeleton className="h-4 w-3/4"/>
                <Skeleton className="h-4 w-1/2"/>
              </div>) : (<div key={index} className="flex gap-4">
    ...
    return (<div className="space-y-4">
      ...
      <div className={viewMode === "grid"
            ? "grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4"
            : "space-y-4"}>
        {filteredProducts.map((product) => viewMode === "grid" ? (<ProductCard key={product.slug} product={product} isInitiallySaved={savedProductIds?.includes(product.id)}/>) : (<ProductCardList key={product.slug} product={product} isInitiallySaved={savedProductIds?.includes(product.id)}/>))}
      </div>
    </div>);
}
