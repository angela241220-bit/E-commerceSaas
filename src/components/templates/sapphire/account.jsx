...
const NavLink = ({ href, page, children, }) => {
    ...
    const isActive = currentPage === href.split("=")[1] ||
        (page === null && href.includes("home"));
    return (<Link href={`/account?page=${href.split("=")[1] || "home"}`}>
      <li className={`p-2 rounded-md ${isActive ? "bg-muted font-semibold" : "hover:bg-muted/50"}`}>
        {children}
      </li>
    </Link>);
};
const Account = ({ store, user }) => {
    ...
    return (<section className="flex flex-col min-h-screen dark:bg-[#1e1b4b] dark:text-white bg-white pt-10">
      ...
      <div className="md:hidden mt-16 container">
        <DropdownMenu>
          ...
          <DropdownMenuContent className="w-56">
            ...
            {menuItems.map((item) => (<Link href={`/account?page=${item.page}`} key={item.page}>
      ...
      <section className="flex-1 flex container mx-auto gap-4 mt-4 md:mt-16">
        <div className="w-1/3 hidden md:block h-fit mt-2">
          <div className="p-2">
            <ul>
              {menuItems.map((item) => (<NavLink href={`/account?page=${item.page}`} page={page} key={item.page}>
                  {item.label}
                </NavLink>))}
            </ul>
          </div>
