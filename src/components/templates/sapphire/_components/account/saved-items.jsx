...
import { getSavedProducts, unsaveProduct, } from "@/actions/store/public/saved/products";
...
const SavedItems = () => {
    ...
    const unsaveMutation = useMutation({
        mutationFn: (productId) => unsaveProduct(productId),
    ...
    if (isLoading) {
        ...
        <CardContent>
          <div className="space-y-4">
            {[...Array(3)].map((_, i) => (<div key={i} className="flex items-center space-x-4 border-b pb-4">
    ...
    return (<Card className="bg-gradient-to-r from-blue-900/40 via-purple-800/40 to-purple-900/40 backdrop-blur-md text-white">
      <CardHeader>
        <CardTitle>Saved Items</CardTitle>
        <CardDescription>View and manage your saved items.</CardDescription>
      </CardHeader>
      <CardContent>
        {savedProducts.length === 0 ? (<p className="text-muted-foreground pt-4">You have no saved items.</p>) : (<div className="space-y-6">
                ...
                return (<div key={item.id} className="flex flex-col sm:flex-row items-start sm:items-center space-y-4 sm:space-y-0 sm:space-x-4 border-b pb-6 last:border-b-0">
                  ...
                  <div className="flex items-center gap-2 w-full sm:w-auto">
                    {cartItem ? (<div className="flex items-center gap-2">
                        <Button size="sm" variant="outline" className="h-9 w-9 p-0" onClick={() => handleDecrement(item.slug)}>
                          <Minus className="h-4 w-4"/>
                        </Button>
                        <span className="text-sm font-medium w-6 text-center">
                          {cartItem.quantity}
                        </span>
                        <Button size="sm" variant="outline" className="h-9 w-9 p-0" onClick={() => handleIncrement(item.slug)} disabled={item.trackQuantity &&
                            cartItem.quantity >= item.inStock}>
                          <Plus className="h-4 w-4"/>
                        </Button>
                      </div>) : (<Button size="sm" className="w-full sm:w-auto" onClick={() => handleAddToCart(item)} disabled={item.trackQuantity && item.inStock <= 0}>
                    ...
                    <Button size="sm" variant="outline" onClick={() => handleRemoveItem(item.id)} disabled={unsaveMutation.isPending}>
