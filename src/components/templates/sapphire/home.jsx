...
export default function Home({ store }) {
    ...
    return (<div className="dark:bg-[#1e1b4b] dark:text-white bg-white">
      <div className="min-h-screen ">
        <Navbar storeSlug={store.slug} storeName={store.name}/>
        {store.customisations[0].productWheelSettings.show && (<Hero storeId={store.id} circleTime={store.customisations[0].productWheelSettings.circleTime || 3} productCount={store.customisations[0].productWheelSettings.productCount || 6} categoryId={store.customisations[0].productWheelSettings.categoryId || "all"}/>)}
        <div className={store.customisations[0].productWheelSettings.show ? "" : "pt-16"}>
        ...
        <div className="container mx-auto px-4 flex flex-col gap-8 py-16">
          ...
          <GenericProductList viewMode="grid" storeId={store.id} limit={12} categoryId="all"/>
        </div>
      </div>
