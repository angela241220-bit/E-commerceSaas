        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2">
            ...
            <div className="space-y-4">
              {items.map((item) => (<Card key={item.id} className="p-4 bg-gradient-to-r from-blue-900/40 via-purple-800/40 to-purple-900/40 backdrop-blur-md text-white">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex items-center mt-4">
                        <div className="flex items-center border rounded">
                          <button onClick={() => updateItemQuantity(item.id, Math.max(1, item.quantity - 1))} className="p-2 hover:bg-gray-100" disabled={item.quantity <= 1}>
                            <Minus className="w-4 h-4"/>
                          </button>
                          <span className="px-4 py-2 text-center min-w-[3rem]">
                            {item.quantity}
                          </span>
                          <button onClick={() => updateItemQuantity(item.id, item.quantity + 1)} className="p-2 hover:bg-gray-100" disabled={item.inStock !== undefined &&
                item.quantity >= item.inStock}>
                            <Plus className="w-4 h-4"/>
                          </button>
                        </div>
                        {item.trackQuantity && item.inStock !== undefined && (<span className="ml-4 text-sm text-gray-500">
          ...
          <div className="lg:col-span-1">
            <Card className="p-6 bg-gradient-to-r from-blue-900/40 via-purple-800/40 to-purple-900/40 backdrop-blur-md text-white">
