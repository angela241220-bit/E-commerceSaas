...
export default function TagsPage({ store }) {
    ...
    return (<section className="min-h-screen flex flex-col dark:bg-[#1e1b4b] dark:text-white bg-white pt-10">
      ...
      <div className="container mx-auto flex-1 mt-16 px-4">
        <div className="flex flex-col gap-4">
          ...
          {isLoading ? (<div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
            ...
            </div>) : error ? (<div className="text-red-500">Failed to load tags</div>) : (<div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
              {tags?.map((tag) => (<Button asChild key={tag.id} aria-label={tag.name}>
