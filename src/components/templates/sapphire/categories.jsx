...
const Categories = ({ store, categories }) => {
    return (<section className="dark:bg-[#1e1b4b] dark:text-white bg-white flex flex-col min-h-screen">
      ...
      <div className="max-w-7xl xl:mx-auto mt-20 w-full flex-1">
        <section className="container max-w-5xl mx-auto px-4 py-16">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 w-full">
            {categories.map((category) => (<Link href={`/categories/${category.slug}`} key={category.id}>
                <Card className="group w-full h-64 relative overflow-hidden transition-all duration-300 hover:shadow-xl bg-sidebar" style={{
