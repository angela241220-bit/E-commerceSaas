...
const AddToCart = ({ product }) => {
    ...
    return (<>
      {!cartItem ? (<Button size="sm" className="w-full gap-1 text-xs" onClick={handleAddToCart} disabled={isOutOfStock}>
          <ShoppingCart className="h-3 w-3"/>
          {isOutOfStock ? "Out of Stock" : "Add to Cart"}
        </Button>) : (<div className="flex items-center justify-between w-full">
          <Button size="sm" variant="secondary" className="h-8 w-8 p-0 text-black" onClick={(e) => {
                e.preventDefault();
                e.stopPropagation();
                handleDecrement();
            }}>
