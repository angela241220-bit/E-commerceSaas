...
import { getSavedAddresses, createSavedAddress, } from "@/actions/customers/saved-addresses";
...
export default function Checkout({ shippingZones, customer, cardBg, store, }) {
    ...
    return (<section className="container mx-auto py-8 md:px-6 mt-10">
      <div className="grid md:grid-cols-2 gap-8">
        <div className="space-y-6">
          {savedAddresses.length > 0 && showSavedAddresses && (<SavedAddresses addresses={savedAddresses} onSelectAddress={handleSavedAddressSelect} selectedAddressId={selectedSavedAddress} cardBg={cardBg}/>)}

          {showSavedAddresses && savedAddresses.length > 0 && (<div className="text-center">
              <Button type="button" variant="outline" onClick={() => setShowSavedAddresses(false)}>
                Enter New Address
              </Button>
            </div>)}
          ...
          <Card className={cn(cardBg)}>
            ...
            <CardContent>
              {shippingZones.length > 0 ? (<SearchableSelect options={shippingZoneOptions} value={selectedZoneId} onChange={setSelectedZoneId} placeholder="Select a shipping zone" searchPlaceholder="Search shipping zones..." emptyMessage="No shipping zones found" disabled={isPending}/>) : (<div className="text-center p-4 border rounded-md bg-muted/20">
          ...
          {(!showSavedAddresses || savedAddresses.length === 0) && (<Card className={cn(cardBg)}>
              <CardHeader>
                <div className="flex items-center justify-between">
                  <CardTitle>Shipping Information</CardTitle>
                  {savedAddresses.length > 0 && (<Button type="button" variant="outline" size="sm" onClick={() => setShowSavedAddresses(true)}>
                      Use Saved Address
                    </Button>)}
                </div>
              </CardHeader>
                ...
                <Form {...form}>
                  <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                    <FormField control={form.control} name="address" render={({ field }) => (<FormItem>
                          ...
                          <FormControl>
                            <Textarea disabled={isPending} placeholder="123 Main St, Anytown, USA" {...field}/>
                          </FormControl>
                          <FormMessage />
                        </FormItem>)}/>
                    <FormField control={form.control} name="primaryPhone" render={({ field }) => (<FormItem>
                          <FormLabel>Primary Phone Number</FormLabel>
                          <FormControl>
                            <Input disabled={isPending} type="tel" placeholder="+1234567890" {...field}/>
                          </FormControl>
                          <FormMessage />
                        </FormItem>)}/>
                    <FormField control={form.control} name="secondaryPhone" render={({ field }) => (<FormItem>
                          <FormLabel>
                            Secondary Phone Number (Optional)
                          </FormLabel>
                          <FormControl>
                            <Input disabled={isPending} type="tel" placeholder="+0987654321" {...field}/>
                          </FormControl>
                          <FormMessage />
                        </FormItem>)}/>
                    <FormField control={form.control} name="additionalInfo" render={({ field }) => (<FormItem>
                          <FormLabel>
                            Additional Information (Optional)
                          </FormLabel>
                          <FormControl>
                            <Textarea disabled={isPending} placeholder="Additional information" {...field}/>
                          </FormControl>
                          <FormMessage />
                        </FormItem>)}/>

                    {/* Save Address Option */}
                    <FormField control={form.control} name="saveAddress" render={({ field }) => (<FormItem className="flex flex-row items-start space-x-3 space-y-0">
                          <FormControl>
                    ...
                    {form.watch("saveAddress") && (<FormField control={form.control} name="addressLabel" render={({ field }) => (<FormItem>
                            <FormLabel>Address Label</FormLabel>
                            <FormControl>
                              <Input disabled={isPending} placeholder="e.g., Home, Work, Office" {...field}/>
                            </FormControl>
                            <FormMessage />
                          </FormItem>)}/>)}
                  </form>
