...
const NavUser = ({ storeSlug, variant = "outline", bgColor = "bg-card", }) => {
    const { customer } = useCustomerSession();
    if (!customer)
        return (<Button variant={variant} asChild>
        <Link href="/sign-in">
          <User />
        </Link>
      </Button>);
      ...
      <DropdownMenuContent className={bgColor}>
        <DropdownMenuLabel>
          <div className="flex items-center gap-2">
            <div>
              {customer.image ? (<Image src={customer.image} alt="User" className="h-16 w-16 rounded-full" width={500} height={500}/>) : (<Avatar className="h-16 w-16">
                  <AvatarFallback>
                    {customer.name.charAt(0) + customer.name.charAt(1)}
                  </AvatarFallback>
                </Avatar>)}
