...
import { getAllCustomerSession, revokeCustomerSession, } from "@/actions/customer";
...
export const SessionManager = ({ className }) => {
    ...
    const getDeviceIcon = (userAgent) => {
        ...
        if (lowercasedUA.includes("mobile"))
            return <PiDeviceMobile />;
        if (lowercasedUA.includes("windows") ||
    ...
    if (isLoading) {
        ...
        <CardContent className="space-y-4">
          {[...Array(3)].map((_, i) => (<div key={i} className="flex items-center justify-between p-2">
    ...
    return (<Card className={cn(className)}>
      ...
      <Table>
        ...
        <TableBody>
          {data?.sessions.map((session, index) => (<TableRow key={index}>
              <TableCell>
                <div className="flex items-center gap-4">
                ...
                : "Unknown Device"}
                      ...
                      {session.id === data?.currentSession?.id && (<Badge>Current session</Badge>)}
                    </div>
                  </div>
                </div>
              </TableCell>
              ...
              <TableCell>
                {session.id === data?.currentSession?.id ? (<span className="text-green-600 font-semibold">
                    Current Session
                  </span>) : (<Button variant={"outline"} onClick={() => mutate(session.id)} disabled={isPending}>
