...
const ReceiptDocument = ({ order, store, watermark, }) => (<Document>
    {/* Watermark - positioned behind content */}
    {watermark && (<Text fixed style={{
            position: "absolute",
            top: "50%",
            left: "50%",
    ...
    <Page size="A4" style={styles.page}>
      ...
      <View style={styles.section}>
        ...
        {order.trackingNumber && (<View style={styles.row}>
      ...
      <View style={styles.section}>
        ...
        {order.customer.phone && (<View style={styles.row}>
      ...
      {order.shippingInfo && (<View style={styles.section}>
          ...
          {order.shippingInfo.phoneNumber && (<Text style={styles.addressText}>
      ...
      <View style={styles.section}>
        ...
        <View style={styles.itemsTable}>
          ...
          {/* Table Rows */}
          {order.items.map((item) => (<View key={item.id} style={styles.tableRow}>
              <View style={styles.productCol}>
                <Text style={styles.tableCell}>{item.productName}</Text>
                {item.variantDetails && (<Text style={[
                styles.tableCell,
...
export const PDFReceipt = ({ order, store, variant = "button", size = "default", className = "", watermark, }) => {
    const filename = `receipt-${order.paymentReference}-${new Date().toISOString().split("T")[0]}.pdf`;
    return (<PDFDownloadLink document={<ReceiptDocument order={order} store={store} watermark={watermark}/>} fileName={filename}>
      {({ blob, url, loading, error }) => {
            // TODO: Remove this
            console.log(blob, url);
            if (variant === "link") {
                return (<span className={`inline-flex items-center gap-2 text-blue-600 hover:text-blue-800 cursor-pointer ${className}`}>
              {loading ? (<>
            ...
            return (<Button variant="outline" size={size} disabled={loading || !!error} className={className}>
            {loading ? (<>
