...
export function ResetPasswordForm({ className, ...props }) {
    ...
    function onSubmit(values) {
        if (invalidToken || !token) {
            ...
            toast.error(invalidToken || "Password reset token is missing or invalid.");
            return;
        }
    ...
    return (<Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className={cn("flex flex-col gap-6", className)} {...props}>
        ...
        <div className="grid gap-6">
          <FormField control={form.control} name="password" disabled={isPending || !!invalidToken} render={({ field }) => (<FormItem className="grid gap-2">
          ...
          <FormField control={form.control} name="confirmPassword" disabled={isPending || !!invalidToken} render={({ field }) => (<FormItem className="grid gap-2">
          ...
          <button type="submit" className="mt-4 group inline-flex items-center justify-center gap-3 text-xs uppercase tracking-widest border border-zinc-900 dark:border-zinc-100 bg-zinc-900 dark:bg-zinc-100 text-white dark:text-zinc-900 px-8 py-4 hover:bg-transparent hover:text-zinc-900 dark:hover:bg-transparent dark:hover:text-zinc-100 transition-colors duration-300 w-full disabled:opacity-50 disabled:cursor-not-allowed" disabled={isPending ||
            !!invalidToken ||
            (process.env.NODE_ENV === "production" && !captchaToken)}>
