...
import { order, storeVisit, orderItem, product, category, customer, } from "@/db/schema";
import { and, eq, gte, lt, sum, count, sql, isNotNull, desc, } from "drizzle-orm";
export const getSalesChartData = async (storeId, days) => {
    ...
    const salesData = await db
        .select({
        date: sql `DATE(${order.createdAt})`.as("date"),
        totalSales: sum(order.total).as("totalSales"),
        orderCount: count(order.id).as("orderCount"),
    })
        .from(order)
        .where(and(eq(order.storeId, storeId), eq(order.paymentStatus, "PAID"), gte(order.createdAt, startDate), lt(order.createdAt, now)))
        .groupBy(sql `DATE(${order.createdAt})`)
        .orderBy(sql `DATE(${order.createdAt})`);
    const visitorsData = await db
        .select({
        date: sql `DATE(${storeVisit.createdAt})`.as("date"),
        visitorCount: count(storeVisit.id).as("visitorCount"),
    })
        .from(storeVisit)
        .where(and(eq(storeVisit.storeId, storeId), gte(storeVisit.createdAt, startDate), lt(storeVisit.createdAt, now)))
        .groupBy(sql `DATE(${storeVisit.createdAt})`)
        .orderBy(sql `DATE(${storeVisit.createdAt})`);
    const visitorsMap = new Map(visitorsData.map((v) => [v.date, Number(v.visitorCount)]));
    const chartData = [];
...
export const getCategoryPerformanceData = async (storeId, days) => {
    const session = await serverAuth();
    if (!session?.user?.id) {
        throw new Error("Unauthorized");
    }
    ...
    const categoryPerformance = await db
        ...
        totalSales: sql `SUM(${orderItem.price} * ${orderItem.quantity})`.as("totalSales"),
    ...
    const totalSales = categoryPerformance.reduce((acc, cat) => acc + Number(cat.totalSales || 0), 0);
...
export const getRecentOrdersData = async (storeId, limit = 5) => {
    const session = await serverAuth();
    if (!session?.user?.id) {
        throw new Error("Unauthorized");
    }
...
export const getTopProductsData = async (storeId, days, limit = 5) => {
    const session = await serverAuth();
    if (!session?.user?.id) {
        throw new Error("Unauthorized");
    }
    ...
    const topProducts = await db
        ...
        categoryName: category.name,
        totalRevenue: sql `SUM(${orderItem.price} * ${orderItem.quantity})`.as("totalRevenue"),
        totalQuantitySold: sql `SUM(${orderItem.quantity})`.as("totalQuantitySold"),
        orderCount: count(sql `DISTINCT ${order.id}`).as("orderCount"),
        ...
        .innerJoin(category, eq(product.categoryId, category.id))
        .where(and(eq(order.storeId, storeId), eq(order.paymentStatus, "PAID"), gte(order.createdAt, startDate), lt(order.createdAt, now)))
        .groupBy(product.id, product.name, product.slug, category.name, product.inStock, product.trackQuantity)
        .orderBy(sql `SUM(${orderItem.price} * ${orderItem.quantity}) DESC`)
        .limit(limit);
...
export const getVisitorAnalyticsData = async (storeId, days) => {
    const session = await serverAuth();
    if (!session?.user?.id) {
        throw new Error("Unauthorized");
    }
    ...
    const dailyVisitorsMap = new Map(dailyVisitors.map((item) => [item.date, Number(item.visitors)]));
    ...
    const osBreakdown = await db
        ...
        .from(storeVisit)
        .where(and(eq(storeVisit.storeId, storeId), gte(storeVisit.createdAt, startDate), lt(storeVisit.createdAt, now), isNotNull(storeVisit.os)))
        .groupBy(storeVisit.os)
        .orderBy(desc(count()));
    const totalVisits = osBreakdown.reduce((sum, item) => sum + Number(item.visits), 0);
