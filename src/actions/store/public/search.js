...
export const searchStore = async (params) => {
    ...
    const [categories, productsByNameOrDesc, productsByQueryTag, productsByTagFilter,] = await Promise.all([
        // 1. Find categories that match the query
        lowerCaseQuery
            ? db
                .select()
                .from(category)
                .where(and(eq(category.storeId, storeId), ilike(category.name, `%${lowerCaseQuery}%`)))
            : Promise.resolve([]),
        ...
        lowerCaseQuery
            ? db
                ...
                .from(product)
                .where(and(eq(product.storeId, storeId), or(ilike(product.name, `%${lowerCaseQuery}%`), ilike(product.description, `%${lowerCaseQuery}%`))))
            : Promise.resolve([]),
        ...
        lowerCaseQuery
            ? db
                ...
                .where(and(eq(tag.storeId, storeId), ilike(tag.name, `%${lowerCaseQuery}%`)))
            : Promise.resolve([]),
    ...
    const products = whereConditions.length > 1
...
export const getSearchPageStoreInfo = async (storeSlug) => {
