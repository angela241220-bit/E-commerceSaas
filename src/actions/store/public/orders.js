...
export const getCustomerOrders = async (params) => {
    ...
    try {
        const { storeId, page = 1, limit = 10, paymentStatus, fulfillmentStatus, minDate, maxDate, search, sort = "createdAt", orderBy = "desc" } = params;
        if (!storeId) {
        ...
        const buildSearchConditions = (baseConditions) => {
            if (search && search.trim()) {
                ...
                return and(...baseConditions, or(ilike(order.paymentReference, searchTerm), ilike(order.trackingNumber, searchTerm)));
            }
        ...
        const [summaryResult] = await db
            .select({
            totalOrders: count(),
            totalSpent: sql `COALESCE(SUM(${order.total}), 0)`,
            pendingOrders: sql `COUNT(CASE WHEN ${order.paymentStatus} = 'PENDING' THEN 1 END)`,
            completedOrders: sql `COUNT(CASE WHEN ${order.paymentStatus} = 'PAID' AND ${order.fulfillmentStatus} = 'DELIVERED' THEN 1 END)`,
        })
            .from(order)
            .where(and(eq(order.storeId, storeId), eq(order.customerId, customer.id)));
 ...
 */
export const getCustomerOrderById = async (orderId, storeId) => {
    const customer = await serverCustomerAuth();
    if (!customer) {
        throw new Error("Customer not authenticated");
    }
    try {
        ...
        const [orderResult] = await db
            ...
            .from(order)
            .where(and(eq(order.id, orderId), eq(order.storeId, storeId), eq(order.customerId, customer.id)));
        if (!orderResult) {
 ...
 */
export const canReorder = async (orderId, storeId) => {
    const customer = await serverCustomerAuth();
    if (!customer) {
        throw new Error("Customer not authenticated");
    }
    try {
        ...
        const orderItemsCheck = await db
            ...
            .leftJoin(product, eq(orderItem.productId, product.id))
            .where(eq(orderItem.orderId, orderId));
        const unavailableItems = orderItemsCheck.filter(item => !item.productStatus ||
