...
export const getTags = async (storeId) => {
    const tags = await db.query.tag.findMany({
        where: eq(tag.storeId, storeId),
    });
    return tags;
};
export const getProductsByTag = async (storeId, tagId) => {
    const products = await db
        ...
        .innerJoin(product, and(eq(productTag.productId, product.id), eq(product.storeId, storeId)))
        .leftJoin(productImage, and(eq(product.id, productImage.productId), eq(productImage.isDefault, true)))
