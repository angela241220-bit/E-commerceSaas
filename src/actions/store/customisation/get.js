...
import { bannerSettings, category, customisations, productWheelSettings, store, } from "@/db/schema";
import { and, eq } from "drizzle-orm";
export const getCustomisation = async (storeId) => {
    ...
    try {
        ...
        const customisationData = await db.query.customisations.findFirst({
            where: and(eq(customisations.storeId, storeInfo.id), eq(customisations.template, storeInfo.template)),
            with: {
        ...
        console.error(error);
        throw new Error(error instanceof Error ? error.message : "Failed to get customisation");
    }
};
