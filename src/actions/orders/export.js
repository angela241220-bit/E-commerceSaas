...
export async function exportOrders(params) {
    ...
    try {
        const { storeId, format = "csv", paymentStatus, fulfillmentStatus, minDate, maxDate, customerId, limit = 1000, } = params;
        if (!storeId) {
        ...
        const exportData = orders.map((orderData) => {
        ...
        throw new Error(error instanceof Error ? error.message : "Failed to export orders");
    }
}
/**
 ...
 */
export async function exportOrdersByDateRange(storeId, startDate, endDate, format = "csv") {
    return exportOrders({
        storeId,
        format,
        minDate: startDate,
 ...
 */
export async function exportOrdersByStatus(storeId, paymentStatus, fulfillmentStatus, format = "csv") {
    return exportOrders({
        storeId,
        format,
        paymentStatus,
 ...
 */
export async function exportRecentOrders(storeId, format = "csv") {
 ...
 */
export async function exportSingleOrder(orderId, storeId, format = "csv") {
    const session = await serverAuth();
    if (!session?.user?.id) {
        throw new Error("Unauthorized");
    }
    try {
        ...
        throw new Error(error instanceof Error ? error.message : "Failed to export order");
    }
}
