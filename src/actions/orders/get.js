...
import { eq, and, gte, lte, ilike, or, desc, asc, count, sql, inArray, } from "drizzle-orm";
...
export async function getStoreOrders(params) {
    ...
    try {
        const { storeId, page = 1, limit = 10, paymentStatus, fulfillmentStatus, minPrice, maxPrice, minDate, maxDate, customerId, search, sort = "createdAt", orderBy = "desc", } = params;
        if (!storeId) {
        ...
        if (paymentStatus &&
            ...
            whereConditions.push(eq(order.paymentStatus, paymentStatus));
        }
        if (fulfillmentStatus &&
            ["PROCESSING", "SHIPPED", "DELIVERED", "CANCELLED"].includes(fulfillmentStatus)) {
            whereConditions.push(eq(order.fulfillmentStatus, fulfillmentStatus));
        }
        ...
        const buildSearchConditions = (baseConditions) => {
            if (search && search.trim()) {
                const searchTerm = `%${search.trim()}%`;
                return and(...baseConditions, or(ilike(order.paymentReference, searchTerm), ilike(order.trackingNumber, searchTerm), ilike(customer.name, searchTerm), ilike(customer.email, searchTerm)));
            }
        ...
        const [summaryResult] = await db
            .select({
            totalOrders: count(),
            totalRevenue: sql `COALESCE(SUM(${order.total}), 0)`,
            pendingOrders: sql `COUNT(CASE WHEN ${order.paymentStatus} = 'PENDING' THEN 1 END)`,
            paidOrders: sql `COUNT(CASE WHEN ${order.paymentStatus} = 'PAID' THEN 1 END)`,
            processingOrders: sql `COUNT(CASE WHEN ${order.fulfillmentStatus} = 'PROCESSING' THEN 1 END)`,
            shippedOrders: sql `COUNT(CASE WHEN ${order.fulfillmentStatus} = 'SHIPPED' THEN 1 END)`,
            deliveredOrders: sql `COUNT(CASE WHEN ${order.fulfillmentStatus} = 'DELIVERED' THEN 1 END)`,
        })
            .from(order)
 ...
 */
export async function getOrderById(orderId, storeId) {
    const session = await serverAuth();
    if (!session?.user?.id) {
        throw new Error("Unauthorized");
    }
 ...
 */
export async function getOrderStats(storeId, days) {
    const session = await serverAuth();
    if (!session?.user?.id) {
        throw new Error("Unauthorized");
    }
    try {
        ...
        const [stats] = await db
            ...
            totalOrders: count(),
            totalRevenue: sql `COALESCE(SUM(${order.total}), 0)`,
            averageOrderValue: sql `COALESCE(AVG(${order.total}), 0)`,
            pendingOrders: sql `COUNT(CASE WHEN ${order.paymentStatus} = 'PENDING' THEN 1 END)`,
            paidOrders: sql `COUNT(CASE WHEN ${order.paymentStatus} = 'PAID' THEN 1 END)`,
            failedOrders: sql `COUNT(CASE WHEN ${order.paymentStatus} = 'FAILED' THEN 1 END)`,
            refundedOrders: sql `COUNT(CASE WHEN ${order.paymentStatus} = 'REFUNDED' THEN 1 END)`,
            processingOrders: sql `COUNT(CASE WHEN ${order.fulfillmentStatus} = 'PROCESSING' THEN 1 END)`,
            shippedOrders: sql `COUNT(CASE WHEN ${order.fulfillmentStatus} = 'SHIPPED' THEN 1 END)`,
            deliveredOrders: sql `COUNT(CASE WHEN ${order.fulfillmentStatus} = 'DELIVERED' THEN 1 END)`,
            cancelledOrders: sql `COUNT(CASE WHEN ${order.fulfillmentStatus} = 'CANCELLED' THEN 1 END)`,
        })
            .from(order)
            .where(and(...whereConditions));
        return {
