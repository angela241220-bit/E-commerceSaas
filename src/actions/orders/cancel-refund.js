 ...
 * Quick cancel order
 */
export async function quickCancelOrder(orderId, storeId, reason) {
    return cancelOrder({
        orderId,
        storeId,
        reason,
 ...
 * Process full refund
 */
export async function processFullRefund(orderId, storeId, reason, restoreInventory = false) {
    return refundOrder({
        orderId,
        storeId,
        reason,
 ...
 */
export async function bulkCancelOrders(orderIds, storeId, reason) {
    const session = await serverAuth();
    if (!session?.user?.id) {
        throw new Error("Unauthorized");
    }
