...
export async function bulkUpdateOrderStatus(params) {
    ...
    try {
        ...
        const existingOrders = await db
            ...
            .from(order)
            .where(and(inArray(order.id, orderIds), eq(order.storeId, storeId)));
        ...
        if (foundOrderIds.length > 0) {
            await db
                .update(order)
                .set(updateData)
                .where(and(inArray(order.id, foundOrderIds), eq(order.storeId, storeId)));
 ...
 */
export async function bulkMarkAsShipped(orderIds, storeId) {
 ...
 */
export async function bulkMarkAsDelivered(orderIds, storeId) {
    return bulkUpdateOrderStatus({
        orderIds,
        storeId,
 ...
 */
export async function bulkMarkPaymentReceived(orderIds, storeId) {
    return bulkUpdateOrderStatus({
        orderIds,
        storeId,
        paymentStatus: "PAID"
 ...
 */
export async function archiveOldOrders(storeId, daysBefore = 365) {
    const session = await serverAuth();
    if (!session?.user?.id) {
        throw new Error("Unauthorized");
    }
    try {
        ...
        const oldOrders = await db
            .select({ id: order.id })
            .from(order)
            .where(and(eq(order.storeId, storeId), eq(order.fulfillmentStatus, "DELIVERED"), sql `${order.deliveredAt} < ${cutoffDate}`));
 ...
 */
export async function getBulkOperationStats(storeId, days = 30) {
    const session = await serverAuth();
    if (!session?.user?.id) {
        throw new Error("Unauthorized");
    }
    try {
        ...
        const stats = await db
            ...
            needsTrackingOrders: sql `COUNT(CASE WHEN ${order.fulfillmentStatus} = 'SHIPPED' AND ${order.trackingNumber} IS NULL THEN 1 END)`,
        })
            .from(order)
            .where(and(eq(order.storeId, storeId), gte(order.createdAt, cutoffDate)));
        return {
            success: true,
            stats: stats[0],
