 ...
 */
export async function updateTrackingInfo(params) {
 ...
 */
export async function addTrackingNumber(orderId, storeId, trackingNumber, shippingProvider) {
 ...
 */
export async function removeTrackingInfo(orderId, storeId) {
    const session = await serverAuth();
    if (!session?.user?.id) {
        throw new Error("Unauthorized");
    }
 ...
 */
export async function bulkUpdateTracking(orders, storeId) {
    const session = await serverAuth();
    if (!session?.user?.id) {
        throw new Error("Unauthorized");
    }
    try {
        ...
        for (const orderUpdate of orders) {
            try {
                const result = await addTrackingNumber(orderUpdate.orderId, storeId, orderUpdate.trackingNumber, orderUpdate.shippingProvider);
                results.push(result);
            }
            catch (error) {
                errors.push({
