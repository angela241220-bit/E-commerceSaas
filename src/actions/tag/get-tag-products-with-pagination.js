 ...
 */
export const getTagProductsWithPagination = async ({ tagId, storeId, limit = 20, offset = 0, active, }) => {
    try {
        if (!tagId || !storeId)
        ...
        const filteredProducts = products.filter((product) => product.tags.some((pt) => pt.tagId === tagId));
        return {
