...
import db from "@/db";
export const updateTag = async (values) => {
    try {
        ...
        const storeRecord = await db
            .select()
            .from(store)
            .where(and(eq(store.id, values.storeId), eq(store.merchantId, values.merchantId)))
            .limit(1);
        ...
        const duplicateTag = await db
            .select()
            .from(tag)
            .where(and(eq(tag.slug, tagSlug), eq(tag.storeId, values.storeId), ne(tag.id, values.id) // Exclude current tag
        ))
            .limit(1);
