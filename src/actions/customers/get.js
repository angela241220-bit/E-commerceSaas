...
export const getCustomers = async ({ storeId, page = 1, limit = 10, search, }) => {
    ...
    const searchConditions = search
        ? or(like(customer.name, `%${search}%`), like(customer.email, `%${search}%`), like(customer.phone, `%${search}%`))
        : undefined;
    ...
    const [{ totalCount }] = await db
        ...
        .from(customer)
        .where(whereCondition);
    const customerStats = await Promise.all(customers.map(async (cust) => {
...
export const getCustomerDetails = async (storeId, customerId) => {
    const session = await serverAuth();
    if (!session?.user) {
        throw new Error("Unauthorized");
    }
    ...
    const orderHistory = await db
        ...
        .where(and(eq(order.customerId, customerId), eq(order.storeId, storeId)))
        .groupBy(order.id, order.paymentStatus, order.fulfillmentStatus, order.total, order.createdAt)
        .orderBy(desc(order.createdAt))
        .limit(10);
    return {
        id: customerInfo.id,
