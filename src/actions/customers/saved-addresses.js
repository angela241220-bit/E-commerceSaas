...
export async function updateSavedAddress(data) {
    try {
        const customer = await serverCustomerAuth();
        if (!customer) {
            return { error: "Unauthorized" };
        ...
        const existingAddress = await db.query.customerSavedAddress.findFirst({
            where: and(eq(customerSavedAddress.id, id), eq(customerSavedAddress.customerId, customer.id)),
        });
        if (!existingAddress) {
        ...
        if (addressData.isDefault) {
            await db
                .update(customerSavedAddress)
                .set({ isDefault: false })
                .where(and(eq(customerSavedAddress.customerId, customer.id), eq(customerSavedAddress.id, id)));
        }
...
export async function deleteSavedAddress(addressId) {
    try {
        ...
        const existingAddress = await db.query.customerSavedAddress.findFirst({
            where: and(eq(customerSavedAddress.id, addressId), eq(customerSavedAddress.customerId, customer.id)),
        });
        if (!existingAddress) {
            return { error: "Address not found" };
        }
