...
export const createShipingZone = async (values, merchantId, storeSlug) => {
    try {
        ...
        const existingZone = await db.query.shippingZone.findFirst({
            where: and(eq(shippingZone.storeId, storeData.id), eq(shippingZone.country, values.country), values.state
                ? eq(shippingZone.state, values.state)
                : isNull(shippingZone.state), values.area
        ...
        if (existingZone) {
            return {
                success: false,
                error: {
                    message: "A shipping zone for this exact location already exists. Please visit the shipping zones page to update it if needed.",
                },
                data: null,
            };
        }
