 ...
 */
export const findShippingZone = async ({ storeId, country, state, area, }) => {
    try {
        // 1. Try to find exact area match first (most specific)
        if (area && state) {
            const areaZone = await db.query.shippingZone.findFirst({
                where: and(eq(shippingZone.storeId, storeId), eq(shippingZone.country, country), eq(shippingZone.state, state), eq(shippingZone.area, area), eq(shippingZone.isActive, true)),
            });
            if (areaZone) {
                return {
                    success: true,
        ...
        if (state) {
            const stateZone = await db.query.shippingZone.findFirst({
                where: and(eq(shippingZone.storeId, storeId), eq(shippingZone.country, country), eq(shippingZone.state, state), isNull(shippingZone.area), // State-level zone (no specific area)
        ...
        const countryZone = await db.query.shippingZone.findFirst({
            where: and(eq(shippingZone.storeId, storeId), eq(shippingZone.country, country), isNull(shippingZone.state), // Country-level zone (no specific state)
 ...
 */
export const getStoreShippingZones = async (storeId) => {
    try {
        const zones = await db.query.shippingZone.findMany({
            where: and(eq(shippingZone.storeId, storeId), eq(shippingZone.isActive, true)),
