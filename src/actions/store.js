"use server";
import db from "@/db";
import { bannerSettings, customisations, productWheelSettings, store, } from "@/db/schema";
import { serverAuth } from "@/lib/server-auth";
import { slugify } from "@/lib/utils";
import { StoreSchema } from "@/schemas";
import { and, eq } from "drizzle-orm";
import { FeatureLimitService } from "@/services/feature-limit";
export const createStore = async (merchantId, values) => {
    if (!values)
        return { error: "Store info is Required" };
    if (!merchantId)
    ...
    const limitCheck = await FeatureLimitService.canUseFeature(user.user.id, "stores_count", 1);
...
export const getStoreByMerchant = async (storeSlug, id) => {
    if (!storeSlug && !id)
        return { error: "Store ID or Slug is required" };
    try {
        const user = await serverAuth();
        if (!user?.session || !user.user)
            return { error: "Unauthorised!" };
...
export const updateStore = async (storeId, values) => {
    try {
