...
import db from "@/db";
import { store, product, customer, order } from "@/db/schema";
...
const page = async () => {
    ...
    const stores = await db
        ...
        name: store.name,
        slug: store.slug,
        productCount: sql `count(distinct ${product.id})`.mapWith(Number),
        customerCount: sql `count(distinct ${customer.id})`.mapWith(Number),
        orderCount: sql `count(distinct ${order.id})`.mapWith(Number),
    })
        .from(store)
        .leftJoin(product, eq(product.storeId, store.id))
        .leftJoin(customer, eq(customer.storeId, store.id))
        .leftJoin(order, eq(order.storeId, store.id))
