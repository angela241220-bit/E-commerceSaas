...
const CategoryPerformance = ({ storeId, days }) => {
    ...
    return (<div className="bg-background p-6 rounded-lg shadow-sm border">
      ...
      <ResponsiveContainer width="100%" height={300}>
        <PieChart>
          ...
          <Tooltip formatter={(value) => [`${value}%`, "Percentage"]} labelFormatter={(label) => `Category: ${label}`} payload={categoryData.map((entry) => ({
            ...entry,
            payload: entry,
        }))} content={({ active, payload }) => {
            if (active && payload && payload[0]) {
                const data = payload[0].payload;
                return (<div className="bg-background p-3 border rounded-lg shadow-sm">
