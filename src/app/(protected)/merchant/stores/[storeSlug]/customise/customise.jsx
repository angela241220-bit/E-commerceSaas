...
const MAX_FILES = 1;
const Customise = ({ store }) => {
    const { data: customisationsData, isLoading, isError, refetch, } = useQuery({
    ...
    const [initialProductWheelState, setInitialProductWheelState] = useState(productWheelState);
    const [bannerSettingsState, setBannerSettingsState] = useState({
        show: true,
    });
    const [initialBannerSettingsState, setInitialBannerSettingsState] = useState(bannerSettingsState);
    const [templateState, setTemplateState] = useState({
        current: store.template || "default",
    });
    const [initialTemplateState, setInitialTemplateState] = useState(templateState);
    useEffect(() => {
        if (customisationsData?.data) {
            if (customisationsData.data.productWheelSettings) {
                const newProductWheelSettings = {
                    ...
                    categoryId: customisationsData.data.productWheelSettings.categoryId || "all",
                };
    ...
    const productWheelHasChanged = JSON.stringify(productWheelState) !==
        JSON.stringify(initialProductWheelState);
    const bannerSettingsHasChanged = JSON.stringify(bannerSettingsState) !==
        JSON.stringify(initialBannerSettingsState);
    const templateHasChanged = JSON.stringify(templateState) !== JSON.stringify(initialTemplateState);
    if (isLoading)
    ...
    return (<section className="px-5 mt-10 grid gap-5">
      <div>
      ...
      <Card className="max-w-4xl mx-auto w-full">
        ...
        <CardContent className="grid gap-5">
          <div className="grid gap-4 border rounded-lg p-4 border-dashed border-gray-300">
            ...
            <div className="space-y-4">
              <div>
                ...
                <Select onValueChange={(template) => setTemplateState((s) => ({ ...s, current: template }))} value={templateState.current} disabled={isPending}>
                  <SelectTrigger className="w-48">
                    <SelectValue placeholder="Select a template"/>
                  </SelectTrigger>
                  <SelectContent>
                    {info.templates.map((template) => (<SelectItem key={template} value={template}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {info.templates.map((template) => (<div key={template} className={`border rounded-lg p-4 cursor-pointer transition-all ${templateState.current === template
                ? "border-blue-500 bg-blue-50 border-4"
                : "border-gray-200 hover:border-gray-300"}`} onClick={() => setTemplateState((s) => ({ ...s, current: template }))}>
                    <div className="aspect-video rounded mb-2 flex items-center justify-center" style={{
              ...
              <Button onClick={handleTemplateSave} disabled={isPending || !templateHasChanged} className="w-fit" variant={templateHasChanged ? "default" : "outline"}>
          ...
          <div>
            <div className="flex flex-col gap-4 border rounded-lg p-4 border-dashed border-gray-300">
              <div className="flex flex-col items-center justify-between gap-4">
                ...
                {store.logoUrl && (<Image src={store.logoUrl} alt="Store Logo" width={500} height={500} className="rounded-lg w-40 h-40 object-cover"/>)}
              ...
              <Button type="button" variant="outline" onClick={() => imageInputRef.current?.click()} disabled={isPending || imageUrls.length >= MAX_FILES}>
                Select Image
              </Button>
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-3 gap-4">
                {imageUrls.map((url, index) => (<div key={url} className="relative group">
                    ...
                    <button type="button" disabled={isPending} onClick={() => removeImage(index)} className="absolute top-2 right-2 p-1 bg-red-500 rounded-full text-white opacity-0 group-hover:opacity-100 transition-opacity">
                      <X className="h-4 w-4"/>
                    </button>
                  </div>))}
              </div>
              <Button disabled={isPending || imageUrls.length === 0} onClick={uploadStoreLogo}>
                Save
              </Button>
            </div>
          </div>
          <div className="grid gap-4 border rounded-lg p-4 border-dashed border-gray-300">
            ...
            <Image src="https://afakpghdgtblnxinpzgy.supabase.co/storage/v1/object/public/zynkart-product-images//Screenshot%202025-06-11%20110835.png" alt="Product Wheel" width={500} height={300} className="rounded-lg w-full"/>
            <div className="flex flex-row items-center justify-between rounded-lg border p-3 shadow-sm">
              ...
              <div>
                <Switch checked={productWheelState.show} onCheckedChange={(show) => setProductWheelState((s) => ({ ...s, show }))} disabled={isPending}/>
              </div>
            </div>
            <div>
              ...
              <Input type="number" min={1} max={10} value={productWheelState.circleTime} onChange={(e) => setProductWheelState((s) => ({
            ...s,
            circleTime: parseInt(e.target.value),
        }))} className="w-24" disabled={isPending}/>
            </div>
            <div>
              ...
              <Input type="number" min={1} max={10} value={productWheelState.productCount} onChange={(e) => setProductWheelState((s) => ({
            ...s,
            ...
            <div>
              ...
              <Select onValueChange={(categoryId) => setProductWheelState((s) => ({ ...s, categoryId }))} value={productWheelState.categoryId} disabled={isPending}>
                <SelectTrigger>
                  <SelectValue placeholder="Select a category"/>
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="all">All</SelectItem>
                  {customisationsData?.categories?.map((cat) => (<SelectItem key={cat.id} value={cat.id}>
                      {cat.name}
                    </SelectItem>))}
                </SelectContent>
              </Select>
            </div>
            <Button onClick={handleProductWheelSave} disabled={isPending || !productWheelHasChanged} className="w-fit" variant={productWheelHasChanged ? "default" : "outline"}>
              Save Changes
            </Button>
          </div>
          <div className="grid gap-4 border rounded-lg p-4 border-dashed border-gray-300">
            ...
            {/* TODO: Add actual template image */}
            <Image src="https://afakpghdgtblnxinpzgy.supabase.co/storage/v1/object/public/zynkart-product-images//Screenshot%202025-06-11%20112635.png" alt="Banners" width={500} height={300} className="rounded-lg w-full"/>
            <div>
              <h3 className="text-lg font-medium">Manage Banners</h3>
              <p className="text-sm text-muted-foreground">
                To add new banners or update existing banner go to the{" "}
                <Link className="text-blue-500 hover:underline" href={`/merchant/stores/${store.slug}/banners`}>
                  Banners
                </Link>{" "}
                page.
              </p>
            ...
            <div className="flex flex-row items-center justify-between rounded-lg border p-3 shadow-sm">
              ...
              <div>
                <Switch checked={bannerSettingsState.show} onCheckedChange={(show) => setBannerSettingsState((s) => ({ ...s, show }))} disabled={isPending}/>
              </div>
            </div>
            <Button onClick={handleBannerSettingsSave} disabled={isPending || !bannerSettingsHasChanged} className="w-fit" variant={bannerSettingsHasChanged ? "default" : "outline"}>
              Save Changes
            </Button>
          </div>
        </CardContent>
