...
const EditCategoryForm = ({ storeId, merchantId, storeSlug, category, }) => {
    const [error, setError] = useState("");
    const [success, setSuccess] = useState("");
    const [isPending, startTransition] = useTransition();
    const router = useRouter();
    const [imageUrl, setImageUrl] = useState(category.imageUrl || undefined);
              ...
              <Form {...form}>
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                  <div className="space-y-4">
                    ...
                    {imageUrl ? (<div className="space-y-4">
                        <div className="relative group w-48 h-36">
                          <Image src={imageUrl} fill alt="Category image" className="object-cover rounded-lg"/>
                          <button type="button" onClick={removeImage} className="absolute top-1 right-1 p-1 bg-red-500 rounded-full text-white opacity-80 group-hover:opacity-100 transition-opacity">
                            <X className="h-4 w-4"/>
                          </button>
                        </div>
                        <Button type="button" variant="outline" onClick={() => imageInputRef.current?.click()} disabled={isPending}>
                          Change Image
                        </Button>
