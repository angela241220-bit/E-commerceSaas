...
const ProductsPage = ({ storeData, merchantId }) => {
    ...
    const { allowed: canCreateProduct, limit: productLimit, upgradeRequired, } = useFeatureLimit({
        userId: merchantId,
        featureKey: "products_count",
        requestedAmount: 1,
    });
    ...
    if (isLoading) {
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
          {Array.from({ length: ITEMS_PER_PAGE }).map((_, index) => (<Skeleton key={index} className="h-64 w-full rounded-md"/>))}
        </div>
      </div>);
    }
    if (error) {
        return (<div className="flex flex-col items-center justify-center space-y-4 my-32">
        ...
        <button onClick={() => router.refresh()} className="px-4 py-2 bg-primary text-primary-foreground rounded-lg hover:bg-primary/90 transition-colors">
          Retry
        </button>
      </div>);
    }
    if (!products || products.length === 0) {
        ...
        <div className="flex flex-col items-center gap-4">
          ...
          {productLimit && (<p className="text-xs text-muted-foreground">
              {productLimit === -1
                    ? "Unlimited products"
    ...
    return (<div className="container mx-auto px-4">
      {productLimit && (<div className="mb-6 flex items-center justify-between bg-card p-4 rounded-lg border">
          <div className="flex items-center gap-4">
            ...
            {productLimit > 0 && (<div className="w-32 bg-secondary rounded-full h-2">
                <div className="bg-primary h-2 rounded-full transition-all" style={{
                    width: `${Math.min(((currentProducts || 0) / productLimit) * 100, 100)}%`,
                }}/>
              </div>)}
          </div>
