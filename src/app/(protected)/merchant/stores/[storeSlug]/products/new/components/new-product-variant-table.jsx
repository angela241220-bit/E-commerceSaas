...
const ProductVariants = ({ colors = [], sizes = [], value, onChange, trackQuantity, basePrice, onAddColor, onAddSize, storeId, merchantId, }) => {
    ...
    const updateVariant = (index, field, newValue) => {
        const updatedVariants = value.map((variant, i) => i === index ? { ...variant, [field]: newValue } : variant);
    ...
    return (<div className="space-y-6">
      <div className="space-y-4">
                ...
                <DialogContent>
                  ...
                  <div className="space-y-4">
                    <div className="space-y-2">
                      ...
                      <Input id="colorName" value={newColor.name} onChange={(e) => setNewColor({ ...newColor, name: e.target.value })} placeholder="e.g., Navy Blue"/>
                    </div>
                    <div className="space-y-2">
                      <Label htmlFor="colorValue">Color Value</Label>
                      <div className="flex gap-2">
                        <Input id="colorValue" value={newColor.value} onChange={(e) => setNewColor({ ...newColor, value: e.target.value })} placeholder="#000000"/>
                        <input type="color" value={newColor.value} onChange={(e) => setNewColor({ ...newColor, value: e.target.value })} className="w-12 h-10"/>
                      </div>
                    </div>
                    {colorError && (<p className="text-sm text-red-500">{colorError}</p>)}
                    <Button onClick={handleAddColor} className="w-full" type="button" disabled={!newColor.value || creatingColor}>
                      {creatingColor ? <i>Adding color</i> : "Add Color"}
                    </Button>
                  </div>
                </DialogContent>
              </Dialog>
                ...
                <DialogContent>
                  ...
                  <div className="space-y-4">
                    <div className="space-y-2">
                      ...
                      <Input id="sizeName" value={newSize.name} onChange={(e) => setNewSize({ ...newSize, name: e.target.value })} placeholder="e.g., Small, Medium, or Large"/>
                    </div>
                    <div className="space-y-2">
                      <Label htmlFor="sizeValue">Size Value</Label>
                      <div className="flex gap-2">
                        <Input id="sizeValue" value={newSize.value} onChange={(e) => setNewSize({ ...newSize, value: e.target.value })} placeholder="e,g. S, M, or L"/>
                      </div>
                    </div>
                    {sizeError && (<p className="text-sm text-red-500">{sizeError}</p>)}
                    <Button onClick={handleAddSize} className="w-full" type="button" disabled={!newSize.value || creatingSize}>
                      {creatingSize ? <i>Adding size</i> : "Add Size"}
                    </Button>
                  </div>
                </DialogContent>
              </Dialog>
        ...
        <div className="flex gap-2 flex-wrap">
          {selectedColors.map((colorId) => (<div key={colorId} className="flex items-center gap-1 bg-sidebar px-2 py-1 rounded">
              ...
              <Button type="button" variant="ghost" size="sm" onClick={() => setSelectedColors(selectedColors.filter((id) => id !== colorId))}>
                <X className="h-4 w-4"/>
              </Button>
            </div>))}
          {selectedSizes.map((sizeId) => (<div key={sizeId} className="flex items-center gap-1 bg-sidebar px-2 py-1 rounded">
              {sizes.find((s) => s.id === sizeId)?.name}
              <Button type="button" variant="ghost" size="sm" onClick={() => setSelectedSizes(selectedSizes.filter((id) => id !== sizeId))}>
                <X className="h-4 w-4"/>
              </Button>
            </div>))}
        </div>
      ...
      {value.length > 0 && (<Table>
          ...
          <TableBody>
            {value.map((variant, index) => (<TableRow key={index}>
                {selectedColors.length > 0 && (<TableCell>
                    <div className="flex items-center gap-2">
                      <div className="w-4 h-4 rounded-full border" style={{
                        backgroundColor: colors.find((c) => c.id === variant.colorId)?.value,
                    }}/>
                      {colors.find((c) => c.id === variant.colorId)?.name}
                    </div>
                  </TableCell>)}
                {selectedSizes.length > 0 && (<TableCell>
                ...
                <TableCell>
                  <Input type="number" value={variant.price || basePrice} onChange={(e) => updateVariant(index, "price", Number(e.target.value))} placeholder="Price"/>
                </TableCell>
                {trackQuantity && (<TableCell>
                    <Input type="number" value={variant.inStock || 0} onChange={(e) => updateVariant(index, "inStock", Number(e.target.value))} placeholder="Stock"/>
                  </TableCell>)}
                <TableCell>
