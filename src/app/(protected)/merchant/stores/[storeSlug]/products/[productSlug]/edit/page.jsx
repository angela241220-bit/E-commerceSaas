import db from "@/db";
import { store, category, color, size, productSource, product, tag, } from "@/db/schema";
...
const page = async ({ params, }) => {
    ...
    const existingProduct = await db.query.product.findFirst({
        where: and(eq(product.storeId, storeData[0].id), eq(product.slug, productSlug)),
        with: {
            images: {
    ...
    return (<EditProductForm {...info} storeSlug={storeSlug} merchantId={merchant.user.id}/>);
};
export default page;
