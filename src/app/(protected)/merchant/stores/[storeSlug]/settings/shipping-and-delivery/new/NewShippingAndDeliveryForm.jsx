...
const NewShippingAndDeliveryForm = ({ merchantId, storeSlug, }) => {
    ...
    const onSubmit = (data) => {
        ...
        startTransition(async () => {
            try {
                ...
                if (!result.success) {
                    if (result.error?.message.includes('duplicate key value violates unique constraint "shipping_zone_location_idx"')) {
                        setError("A shipping zone for this exact location already exists. Please visit the shipping zones page to update it if needed.");
                        return;
                    }
                ...
                form.reset();
                router.push(`/merchant/stores/${storeSlug}/settings/shipping-and-delivery`);
            }
            catch (error) {
                console.error(error);
    ...
    return (<div className="min-h-screen bg-background">
      <div className="w-full max-w-3xl mx-auto p-4 pt-20 space-y-6">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
          <Link href={`/merchant/stores/${storeSlug}/settings/shipping-and-delivery`} className="inline-flex items-center text-sm text-muted-foreground hover:text-primary transition-colors mb-6">
              ...
              <Form {...form}>
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                  ...
                  <FormField control={form.control} name="state" render={({ field }) => (<FormItem>
                        ...
                        <FormControl>
                          <SearchableSelect value={field.value} disabled={isPending} onChange={(value) => {
                field.onChange(value);
                setSelectedState(value);
                // Reset LGA when state changes
                form.setValue("area", "");
            }} placeholder="Select state" searchPlaceholder="Search state..." emptyMessage="No state found." options={Object.keys(NIGERIA_STATES).map((state) => ({
                value: state,
                label: state,
            }))}/>
                        </FormControl>
                  ...
                  <FormField control={form.control} name="area" render={({ field }) => (<FormItem>
                        ...
                        <FormControl>
                          <SearchableSelect value={field.value} onChange={field.onChange} disabled={!selectedState || isPending} placeholder="Select LGA" searchPlaceholder="Search LGA..." emptyMessage="No LGA found." options={selectedState
                ...
                : []}/>
                  ...
                  {/* Shipping Cost */}
                  <FormField control={form.control} name="shippingCost" render={({ field }) => (<FormItem>
                        <FormLabel>Shipping Cost (₦)</FormLabel>
                        <FormControl>
                          <Input {...field} disabled={isPending} placeholder="Enter shipping cost" type="number" value={field.value === undefined ? "" : field.value} onChange={(e) => field.onChange(e.target.value
                ...
                : "Enter a valid cost"}
                  ...
                  <div className="flex flex-col gap-4 sm:flex-row sm:justify-end">
                    <Button type="button" variant="outline" disabled={isPending} asChild>
                      <Link href={`/merchant/stores/${storeSlug}/settings/shipping-and-delivery`}>
                        Cancel
                      </Link>
                    </Button>
