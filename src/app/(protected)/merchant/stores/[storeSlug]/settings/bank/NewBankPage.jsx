...
const NewBankPage = ({ storeData, }) => {
    ...
    const handleVerification = (accountNumber, bankCode) => {
    ...
    return (<PageLayout backLink={`/merchant/stores/${storeData.slug}/settings/bank`} backText="Back to Banks" title="Add new bank" description="Set up a new bank for your store">
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
          <FormField control={form.control} name="bankName" render={({ field }) => (<FormItem>
                <FormLabel>Bank</FormLabel>
                <FormControl>
                  <Select disabled={isPending || isLoadingBanks} onValueChange={(value) => {
                ...
                if (selectedBank) {
                    ...
                    <SelectContent>
                      {banks.map((bank) => (<SelectItem key={bank.slug} value={bank.code}>
                          {bank.name}
                        </SelectItem>))}
                    </SelectContent>
                  </Select>
          ...
          <FormField control={form.control} name="accountNumber" render={({ field }) => (<FormItem>
                <FormLabel>Account Number</FormLabel>
                <FormControl>
                  <Input {...field} disabled={isPending} placeholder="0350024645" type="text" className="h-12" onChange={(e) => {
                field.onChange(e);
                ...
                <FormMessage />
                {isVerifying && (<div className="text-sm text-blue-500 mt-1">
                    Verifying account...
                  </div>)}
              </FormItem>)}/>

          <FormField control={form.control} name="accountName" render={({ field }) => (<FormItem>
                <FormLabel>Account Name</FormLabel>
                <FormControl>
                  <Input {...field} disabled={true} placeholder="Account Name" type="text" className="h-12 text-green-400 capitalize font-bold"/>
                </FormControl>
                <FormMessage />
              </FormItem>)}/>
          ...
          <FormField control={form.control} name="country" render={({ field }) => (<FormItem>
                ...
                <FormControl>
                  <Input {...field} disabled={true} type="text" className="h-12"/>
                </FormControl>
                <FormMessage />
              </FormItem>)}/>
