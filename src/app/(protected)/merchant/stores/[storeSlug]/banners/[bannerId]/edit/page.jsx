...
const EditBannerPage = async ({ params, }) => {
    const { storeSlug, bannerId } = await params;
    const session = await serverAuth();
    if (!session?.user || !session.session)
        return redirect(`/sign-in?callbackURL=/merchant/stores/${storeSlug}/banners/${bannerId}/edit`);
    ...
    return (<div className="min-h-screen bg-background">
      <div className="w-full max-w-3xl mx-auto p-4 pt-20 space-y-6">
        ...
        <Card>
          ...
          <CardContent>
            <EditBannerForm banner={bannerInfo} storeData={{ id: storeInfo.id, name: storeInfo.name }} storeSlug={storeSlug} merchantId={session.user.id}/>
          </CardContent>
        </Card>
      </div>
    </div>);
