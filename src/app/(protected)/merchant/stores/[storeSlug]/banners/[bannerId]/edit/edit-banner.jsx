...
const EditBannerForm = ({ banner, storeData, storeSlug, merchantId, }) => {
    ...
    const onSubmit = async (values) => {
        ...
        try {
            ...
            const { error: updateError } = await updateBanner(banner.id, finalValues, merchantId);
            if (updateError) {
          ...
          <div className="space-y-4">
            <div className="space-y-4">
              <div className="flex flex-col gap-4">
                ...
                <input type="file" hidden accept="image/*" ref={imageInputRef} onChange={handleImageChange} disabled={isPending}/>

                <div className="space-y-4">
                  <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-3 gap-4">
                    <div key={imageUrl} className="relative group">
                      <Image src={imageUrl} width={500} height={375} alt={`Banner image`} className="object-cover rounded-lg aspect-[4/3] w-full h-full"/>
                      {isNewImageSelected && (<button type="button" onClick={revertImage} className="absolute top-2 right-2 p-1 bg-red-500 rounded-full text-white opacity-70 hover:opacity-100 transition-opacity">
                          <X className="h-4 w-4"/>
                        </button>)}
                    </div>
                  </div>
            ...
            : "border-gray-300 hover:border-gray-400"} ${isPending
            ...
            <FormField control={form.control} name="title" render={({ field }) => (<FormItem>
                  ...
                  <FormControl>
                    <Input {...field} disabled={isPending} placeholder="e.g., Summer Sale" type="text" className="h-12"/>
                  </FormControl>
                  <FormDescription>
            ...
            <FormField control={form.control} name="description" render={({ field }) => (<FormItem>
                  ...
                  <FormControl>
                    <Textarea {...field} disabled={isPending} placeholder="Banner description" rows={4}/>
                  </FormControl>
                  <FormMessage />
                </FormItem>)}/>
            <FormField control={form.control} name="linkUrl" render={({ field }) => (<FormItem>
                  <FormLabel>Link URL</FormLabel>
                  <FormControl>
                    <Input {...field} disabled={isPending} placeholder="e.g., /products/summer-collection" type="text" className="h-12"/>
                  </FormControl>
                  <FormDescription>
            ...
            <FormField control={form.control} name="isActive" render={({ field }) => (<FormItem className="flex flex-row items-center justify-between rounded-lg border p-4">
