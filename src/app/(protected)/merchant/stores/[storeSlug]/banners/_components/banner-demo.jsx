...
function BannerDemoDefaultTemplate({ banners, storeSlug, }) {
    ...
    return (<div className="relative w-full py-8 px-8">
      <Dialog open={!!bannerToDelete} onOpenChange={(isOpen) => {
            if (!isOpen)
                setBannerToDelete(null);
        }}>
        <DialogContent>
          ...
          {bannerToDelete && (<div className="my-4">
              <p className="font-semibold">{bannerToDelete.title}</p>
              <div className="mt-2 relative aspect-[4/3] w-full rounded-lg overflow-hidden">
                <Image width={500} height={375} src={bannerToDelete.imageUrl || ""} alt={bannerToDelete.title || "Banner image"} className="object-cover w-full h-full"/>
              </div>
            </div>)}
          <DialogFooter>
            <Button variant="outline" onClick={() => setBannerToDelete(null)} disabled={isPending}>
              Cancel
            </Button>
            <Button variant="destructive" onClick={() => bannerToDelete && handleDelete(bannerToDelete.id)} disabled={isPending}>
      ...
      <div className="absolute right-8 top-8 z-10 flex items-center gap-2">
        {currentBanner && (<>
            <Button asChild size="sm" variant="outline">
              <Link href={`/merchant/stores/${storeSlug}/banners/${currentBanner.id}/edit`}>
                <Edit2 className="mr-2 h-4 w-4"/>
                Edit
              </Link>
            </Button>
            <Button size="sm" variant="destructive" onClick={() => setBannerToDelete(currentBanner)} disabled={isPending}>
      ...
      <Carousel setApi={setApi} className="w-full max-w-7xl mx-auto" plugins={[
            Autoplay({
                delay: 3000,
            }),
        ]}>
            ...
            <Link href={banner.linkUrl || ""}>Browse Categories</Link>
                    ...
                    <div className="relative">
                      <div className="aspect-[4/3] rounded-lg overflow-hidden">
                        <Image width={500} height={500} src={banner.imageUrl || ""} alt="Hero Image" className="object-cover w-full h-full"/>
                      </div>
      ...
      <div className="flex justify-center mt-4 sm:mt-6 gap-1 sm:gap-1.5">
        {banners.map((_, index) => (<div key={index} className={`h-1 sm:h-1.5 rounded-full transition-all duration-300 ${index === 0
