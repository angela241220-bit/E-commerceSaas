...
const MAX_FILES = 1;
const NewBannerForm = ({ storeData, storeSlug, merchantId, }) => {
    ...
    const onSubmit = async (values) => {
        ...
        try {
            ...
            const { success, error: createError } = await createBanner(finalValues, merchantId, storeData.id);
            if (createError) {
    ...
    return (<motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
      <Link href={`/merchant/stores/${storeSlug}/banners`} className="inline-flex items-center text-sm text-muted-foreground hover:text-primary transition-colors mb-6">
            ...
            : "Drag and drop banner image here"}
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-3 gap-4">
                  {imageUrls.map((url, index) => (<div key={url} className="relative group">
                      <Image src={url} width={500} height={375} alt={`Banner image ${index + 1}`} className="object-cover rounded-lg aspect-[4/3] w-full h-full"/>
