...
import { getFeatureLimits, deleteFeatureLimit, } from "@/actions/admin/feature-limits";
...
export function FeatureLimitsTable() {
    ...
    const handleDelete = async (planType, featureKey) => {
        try {
    ...
    const formatLimitValue = (value, limitType) => {
        if (value === -1)
            return "Unlimited";
        if (value === 0 && limitType === "boolean")
            return "Disabled";
        if (value === 1 && limitType === "boolean")
            return "Enabled";
    ...
    const getLimitTypeColor = (limitType) => {
        switch (limitType) {
            case "count":
    ...
    return (<div className="space-y-4">
      <Table>
        ...
        <TableBody>
          {limits.map((limit) => (<TableRow key={`${limit.planType}-${limit.featureKey}`}>
              <TableCell>
              ...
              <TableCell>
                <span className={`px-2 py-1 rounded-full text-xs font-medium ${getLimitTypeColor(limit.limitType)}`}>
                  {limit.limitType}
                </span>
              </TableCell>
              <TableCell>
              ...
              <TableCell className="max-w-xs truncate" title={limit.description ?? undefined}>
                {limit.description ?? "-"}
              </TableCell>
                  ...
                  <AlertDialog>
                    ...
                    <AlertDialogContent>
                      ...
                      <AlertDialogFooter>
                        ...
                        <AlertDialogAction onClick={() => handleDelete(limit.planType, limit.featureKey)} className="bg-destructive text-destructive-foreground hover:bg-destructive/90">
                          Delete
                        </AlertDialogAction>
                      </AlertDialogFooter>
                    </AlertDialogContent>
